...
tf.app.flags.DEFINE_bool('multi_scale', True, 'Multi scale')
tf.app.flags.DEFINE_bool('augmentation', True, 'Augmentation')
tf.app.flags.DEFINE_bool('decayed_learning_rate', True, 'Decay the Learning')
tf.app.flags.DEFINE_bool('crelu', True, 'Crelu as activation function')
run_log_dir = os.path.join(FLAGS.log_dir,
                           ('log_improved').format(ep=FLAGS.epochs, wd=FLAGS.weight_decay, lr=FLAGS.learning_rate))
...
def main(_):
    ...
    with tf.variable_scope('model'):
        ...
        if(FLAGS.decayed_learning_rate):
            learning_rate = tf.train.exponential_decay(FLAGS.learning_rate, global_epoch, decay_steps, decay_rate, staircase=False)
        else:
    ...
    wrong_prediction_summary = tf.summary.image('Miss Classified Images', x_image)
    train_summary = tf.summary.merge([loss_summary, accuracy_summary, img_summary, wrong_prediction_summary])
